...
if not load_script then
    out.ting("*** WARNING: Not loading script for campaign " ..
                 campaign_start_file ..
                 " as load_script variable is set to false! Edit lua file at " ..
                 debug.getinfo(1).source .. " to change this back ***");
    return;
end
...
package.path = package.path .. ";data/campaigns/" .. campaign_name ..
                   "/factions/?.lua";
...
cm:register_ui_created_callback(function()
    ...
    if not (local_faction == "") then
        ...
        if load_faction_script(local_faction) and
            load_faction_script(local_faction .. "_intro") then
            dec_tab();
...
function load_faction_script(scriptname)
    ...
    if success then
        ...
        script_error("ERROR: Tried to load faction script " .. scriptname ..
                         " without success - either the script is not present or it is not valid. See error below");
        output("*************");
...
cm:register_first_tick_callback(function()
    if is_function(start_game_for_faction) then
        ...
        script_error(
            "start_game_for_faction() function is being called but hasn't been loaded - the script has gone wrong somewhere else, investigate!");
    end
...
-- �����
ram = require("random_army");

require("bel_common");
require("antonina_incident");
...
-- ALFRED SCRIPTS --
require("spawn_by_year");
require("alfred_turn_manager");
require("army_spawn_scripts");
require("peform_spawn");
-- ALFRED SUBMODS DATA --
require("alfred_submods_data/alfred_coordinates");
require("alfred_submods_data/alfred_faction_keys");
require("alfred_submods_data/alfred_region_keys");
